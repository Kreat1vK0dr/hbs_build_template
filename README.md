...
- Fork repo and `git clone` (making a duplicate) *under _different_ name* :
  ...
  * *THEN* in your command line paste : `git clone git@github.com/<your_username>/hbs_build_template.git`
    - Note that when you `git clone <git URL> ` *without specifying a folder name* a new folder with the same name of the repository you just cloned will be created i.e. *app_template[handlebars]*. If you *want a different folder/repository name* then add ` <new_folder_name>` to the above line, so that it looks like this:
